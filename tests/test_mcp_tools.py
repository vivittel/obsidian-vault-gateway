...
async def test_create_inbox_note_schema_defines_bullet_block_depth_and_checked() -> None:
    # docs/adr/0011-*.md: nesting depth and GFM task-list checkboxes are
    # BulletBlock-only — ProcedureStep's own TextBlock has neither.
    tools = {t.name: t for t in await mcp.list_tools()}
    schema = tools["create_inbox_note"].input_schema
    bullet_schema = schema["$defs"]["BulletBlock"]["properties"]
    assert set(bullet_schema) == {"type", "content", "depth", "checked"}
    assert bullet_schema["depth"]["default"] == 0
    text_block_schema = schema["$defs"]["TextBlock"]["properties"]
    assert "depth" not in text_block_schema
    assert "checked" not in text_block_schema


async def test_create_inbox_note_schema_defines_table_block() -> None:
    # docs/adr/0011-*.md: a body field's rich block sequence can carry a
    # table, generated by the Gateway rather than accepted as raw Markdown.
    tools = {t.name: t for t in await mcp.list_tools()}
    schema = tools["create_inbox_note"].input_schema
    table_schema = schema["$defs"]["TableBlock"]["properties"]
    assert set(table_schema) == {"type", "label", "headers", "alignments", "rows"}
...
async def test_create_inbox_note_body_field_accepts_a_bare_string_or_a_rich_block() -> None:
    ...
    variants = design_items_schema["anyOf"]
