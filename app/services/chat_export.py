...
def _total_block_chars(
    *,
    ...
    = 30, docs/adr/0013-*.md): a bullet-only ``full`` export's schema-level
    ceiling is now ``_MAX_TOPIC_ITEMS`` (50) x ``_MAX_TOPIC_POINT_ITEMS``
    (100) x ``_MAX_LINE_CHARS`` (1,000) = 5,000,000 chars, up from 600,000.
    That figure is still a hard, finite ceiling regardless of
    ``Settings.max_request_bytes`` — it is not itself unbounded — but at
    that setting's *default* (2 MiB, ``app/config.py``) the transport-level
    body-size backstop is what actually rejects a bullet-only payload this
    large before it reaches this budget at all; ``max_request_bytes`` has
    no configured upper bound, so raising it enough would make this
    per-field schema ceiling the binding one again. A paragraph's content
    is NOT analogous to a bullet's: ``ParagraphContent``'s own cap is 8x
    ``Line``'s (see ``_MAX_PARAGRAPH_CHARS``), so leaving it uncounted
    would let a single body field carry far more prose than code/table/
    quote content ever could under the same per-item cap — this is the
